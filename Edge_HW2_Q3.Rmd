...
set.seed(15072) # for reproducibility

train_index <- createDataPartition(df_orig$Churn, p = 0.7, list = FALSE)
df_train <- df_orig[train_index, ]
df_test  <- df_orig[-train_index, ]

# Check dimensions of training and test sets
dim(df_train) # training set dimensions
dim(df_test)  # test set dimensions

# Check churn distribution in training and test sets
table(df_train$Churn)
table(df_test$Churn)

# Proportion of churn in training and test sets
prop.table(table(df_train$Churn))
prop.table(table(df_test$Churn))
```
...
In the logistic regression model, the `SeniorCitizen` coefficient represents the change in the log-odds of churn for senior citizens compared to non-senior citizens, holding all other variables constant. Since the coefficient is positive (0.4615), it means that being a senior citizen increases the likelihood of customer churn. Specifically, the odds of churn for senior citizens are exp(0.4615) ≈ 1.59 times the odds for non-senior citizens, all else being equal. In other words, senior citizens are about 59% more likely to churn than non-senior citizens, controlling for other factors in the model.
